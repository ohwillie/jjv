      ...
      if (options.checkRequired && schema.required) {
        for (i = 0, len = schema.required.length; i < len; i++)
          if (!prop.hasOwnProperty(schema.required[i])) {
            objerrs[schema.required[i]] = {'required': {error: true}};
            malformed = true;
          }
      }
        ...
        if (schema.hasOwnProperty('additionalProperties')) {
          if (typeof schema.additionalProperties === 'boolean') {
            if (!schema.additionalProperties) {
              for (i = 0, len = props.length; i < len; i++) {
                objerrs[props[i]] = {'additional': {error: true}};
                malformed = true;
              }
            }
          } else {
      ...
      if (schema.hasOwnProperty(v) && !handled.hasOwnProperty(v)) {
        if (v === 'format') {
          if (env.fieldFormat.hasOwnProperty(schema[v]) && !env.fieldFormat[schema[v]](prop, schema, object_stack, options)) {
            objerrs[v] = {error: true};
            malformed = true;
          }
        } else {
          if (env.fieldValidate.hasOwnProperty(v)) {
            expected = schema[v].hasOwnProperty('$data') ? resolveObjectRef(object_stack, schema[v].$data) : schema[v];
            if (!env.fieldValidate[v](prop, expected, schema, object_stack, options)) {
              objerrs[v] = {error: true, expected: expected};
              malformed = true;
            }
          }
        }
